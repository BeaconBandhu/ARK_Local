...
import { analyseAnswer, getBaseUrl, getCurrentTopic, isOnline, syncOfflineQueue } from '../services/api';
...
export default function StudentScreen({ navigation }) {
  ...
  const [baseUrl, setBaseUrl] = useState('http://192.168.1.100:8000');
  ...
  async function bootstrap() {
    const [storedId, storedName, url] = await Promise.all([
  ...
  if (!setupDone && !studentId) {
    return (
      <View style={styles.setup}>
        ...
        <TextInput
          style={styles.setupInput}
          placeholder="Server URL (teacher's PC IP)"
          placeholderTextColor="#64748b"
          value={baseUrl}
